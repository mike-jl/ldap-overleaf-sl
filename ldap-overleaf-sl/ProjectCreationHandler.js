...
async function _addExampleProjectFiles(ownerId, projectName, project) {
  ...
  const styleDocLines = await _buildTemplate(
    `${templateProjectDir}/rapportECL2024.cls`,
    ownerId,
    projectName
  );
  await ProjectEntityUpdateHandler.promises.addDoc(
  ...
  const logosFolder = new Folder({ name: "logos" });
  project.rootFolder[0].folders = [imagesFolder, logosFolder];

  const frogPath = path.join(
    __dirname,
    `/../../../templates/project_files/${templateProjectDir}/images/frog.jpg`
  );
  ...
  const blasonPath = path.join(
    __dirname,
    `/../../../templates/project_files/${templateProjectDir}/logos/blason.png`
  );
  await ProjectEntityUpdateHandler.promises.addFile(
    project._id,
    logosFolder._id,
  ...
  const logoPath = path.join(
    __dirname,
    `/../../../templates/project_files/${templateProjectDir}/logos/logo.png`
  );
  await ProjectEntityUpdateHandler.promises.addFile(
    project._id,
    logosFolder._id,
  ...
  const motifPath = path.join(
    __dirname,
    `/../../../templates/project_files/${templateProjectDir}/logos/motif.png`
  );
  await ProjectEntityUpdateHandler.promises.addFile(
    project._id,
    logosFolder._id,
